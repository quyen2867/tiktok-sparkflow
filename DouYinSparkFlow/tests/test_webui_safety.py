...
class WebUiSafetyTests(unittest.TestCase):
    ...
    def test_stale_lock_inspection_does_not_delete_file(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual("", status["staleReason"])
    ...
    def test_login_desktop_timeout_is_wrapped_as_runtime_error(self):
        with patch.object(app_module.urllib.request, "urlopen", side_effect=TimeoutError("timed out")):
            with self.assertRaisesRegex(RuntimeError, "Dịch vụ đăng nhập không khả dụng: timed out"):
    ...
    def test_dashboard_uses_native_manual_login(self):
        dashboard = (Path(app_module.TEMPLATES_DIR) / "dashboard.html").read_text(encoding="utf-8")
        self.assertIn("data-native-login", dashboard)
        self.assertIn("CAPTCHA", dashboard)
        self.assertNotIn("data-login-qr", dashboard)
    ...
    def test_schedule_sync_does_not_write_host_crontab(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertFalse(cron_path.exists())
